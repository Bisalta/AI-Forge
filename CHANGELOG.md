...
- **Branch atada al task key**: nuevo formato `{action}-{KEY}-{desc}` (`action ∈ feat|fix|chore|refactor|docs`) que reemplaza `<MODULO>-<TICKET>` cuando hay Proxima; fallback al viejo formato sin Proxima. Orden estricto: tarea Proxima primero → branch después.
- **Cierre por merge**: subtask → `done` cuando su PR se mergea (no en Feature Ready); tarea madre → `done` cuando todas las subtasks están `done`.
