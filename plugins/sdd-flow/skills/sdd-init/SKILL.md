...
8. **Copiá los scripts del plugin a `SDD/scripts/`** (creando el dir): `sdd-check.sh` (chequeo mecánico del diff), `sdd-run-gates.sh` (corre la escalera y genera la evidencia), `sdd-lint-contract.sh` (closure del contract), `sdd-context-budget.sh` (contexto estático por rol) y `sdd-escalation-tally.sh` (cuenta `SDD/escalations.md` por Clase). Dentro del repo no dependen de que el plugin esté instalado, y CI puede correrlos. **Versionado**: cada script responde `--version`; si ya existen copias, compará — copia más vieja que la del plugin → actualizala avisando; jamás pises una copia modificada localmente sin mostrar el diff.
   - **Si `SDD/escalations.md` no existe, creálo desde `templates/escalations-ledger.md`** (tabla vacía — el backfill de otro repo no aplica acá). Si ya existe, no lo toques: es un ledger append-only con historia propia. *(Sub-ítem de arriba, no un paso `8bis.` — `8bis.` no es marcador válido de lista ordenada en Markdown y rompía la numeración; hallazgo de ronda 2, MINOR m5.)*
