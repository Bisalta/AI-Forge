...
- **AC de detección** (guarda, validación, chequeo de ausencia, sensibilidad — el criterio de clasificación y las corridas que exige están en `standards/quality-gates.md` §10 «Prueba por mutación (AC de detección)», leelo ahí y no lo reconstruyas de memoria): aplicá la mutación que el contract declara debajo del AC, registrá **cada corrida con su comando y su exit code** en el verification report, y dejá la mutación revertida. La mutación se aplica al sistema que el AC vigila, **nunca al test** — aflojar el test para verlo fallar es mitigación prohibida. Si el AC entra en ese criterio y el contract no declara la mutación → **BLOCKED**, preguntá al planner: inventarla vos es cerrar una decisión que el contract dejó abierta.
...
- Escribí tu verification report (`tasks/<slug>/verification/AGENT_<vos>.md`, o `SDD/verification/<branch>.md` single-repo) con `templates/verification-report.md`: referencia al reporte generado + lo que el runner no sabe — **doble corrida del test de reproducción** (bugfix), **las tres corridas de cada AC de detección** con su mutación (`quality-gates.md` §10; el template tiene la sección), smoke `manual-only`, **impact set** (callers de cada símbolo cambiado y cómo quedaron cubiertos), rojos preexistentes de la base.
- Gate sin prerequisito disponible → `[SKIPPED] <prereq>`, nunca "verde".
- **Toda cifra que reportes va con la salida del comando que la produce** (`quality-gates.md` §5), pegada tal cual: casos que pasaron, archivos escaneados, hallazgos, líneas, tiempos. Una cifra copiada de otro documento —de un reporte anterior, del contract, de un mensaje— es una cita, no una medición: re-corré el comando y pegá su salida, o no escribas la cifra. En review, cifra sin su salida es `MAJOR`.
