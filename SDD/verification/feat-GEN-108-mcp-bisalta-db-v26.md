...
- **Branch**: `feat-GEN-108-mcp-bisalta-db` · **Commit**: `eca0a9c` · **Doc**: `SDD/docs/doc_quality_gates.md` (`sha256:56736c3e5b778ea1`) · **Fecha**: 2026-09-25T19:29:43Z
- Tree: `67e2b569bf85104e6d96f21fea4106032e9748cf` — LIMPIO
...
|---|---|---|---|---|---|
| 1 | format / style | — | — | 2026-09-25T19:27:12Z | [SKIPPED] sin comando en el doc (N/A — shfmt no está instalado) |
| 2 | lint | `shellcheck --severity=warning plugins/sdd-flow/scripts/*.sh plugins/sdd-flow/hooks/*.sh plugins/usage-monitor/scripts/*.sh SDD/tests/*.sh SDD/scripts/*.sh` | 0 | 2026-09-25T19:27:13Z | verde |
| 3 | type-check | — | — | 2026-09-25T19:27:14Z | [SKIPPED] sin comando en el doc (N/A — bash no es tipado) |
| 4 | unit tests | `bash SDD/tests/run.sh` | 0 | 2026-09-25T19:27:14Z | verde |
| 5 | integration | — | — | 2026-09-25T19:28:26Z | [SKIPPED] sin comando en el doc (N/A — los tests del harness ya ejercitan los scripts end-to-end) |
| 6 | build | — | — | 2026-09-25T19:28:26Z | [SKIPPED] sin comando en el doc (N/A — el plugin no compila) |
| 7 | e2e | — | — | 2026-09-25T19:28:26Z | [SKIPPED] sin comando en el doc (N/A) |
| 8 | cobertura del diff | — | — | 2026-09-25T19:28:26Z | [SKIPPED] sin comando en el doc (N/A — sin reporte de coverage; se verifica con el binding AC↔test) |
| 9 | security | `bash SDD/tests/secret-scan.sh` | 0 | 2026-09-25T19:28:26Z | verde |
| 10 | smoke manual | — | — | 2026-09-25T19:28:28Z | [SKIPPED] sin comando en el doc (N/A) |
| — | suite completa | `bash SDD/tests/run.sh` | 0 | 2026-09-25T19:28:29Z | verde |
...
La escalera de arriba se corrió sobre `eca0a9c`, con las correcciones de las rondas 1 y 2 de la review de v26. Las mutaciones, la verificación en vivo y la lectura del bundle se corrieron sobre `47a4a69`, el commit que agregó la escalera: el código y los tests son los de `eca0a9c`. Esta versión del report reemplaza a las de `d14b774` y `0ad5262`, que quedan en el historial.

**Una corrida roja antes de ésta.** La primera escalera de v26 (`c85b4ea`) salió con el gate 9 en rojo: el secret-scan encontró en el bundle un patrón con forma de clave de AWS. El commit se encadenó detrás del runner sin mirar su resultado (`RT56`). Se resolvió con `AC59`, y esta escalera la reemplaza. La roja queda en el historial. **La primera versión de `AC59` tenía dos huecos**, y la review de v26 los encontró en la ronda 1. Un `BEGIN` sin `END` apagaba el scan del resto del archivo, y `clave=valor` pasaba por base64. Los dos están corregidos (`9d8dff1`), con las formas 10 a 13 y sus mutaciones. En la ronda 2 apareció un tercero: un `BEGIN` con el bloque ya abierto no lo reabría, así que un PEM truncado seguido de uno completo formaba un solo bloque. Se corrigió en `eca0a9c`, con la forma 14 y la mutación (h).
...
Es el script de v25 con estos cambios. `AC55 (a)` quita ahora la línea de `verify-full`. Hay tres mutaciones nuevas de `AC58` y ocho de `AC59`, de la (a) a la (h), que corren contra `SDD/tests/test_secret_scan.sh`. Y el script exige árbol limpio también al empezar (`RT56`).
...
- mutado:             exit=1 fail=2
      ...
      FAIL  forma13 un certificado con CRLF se reconoce - verde
- verde (restaurado): exit=0 fail=0
...
- mutado:             exit=1 fail=4
      ...
      FAIL  forma11 clave=valor dentro de un certificado cerrado - rojo (no es base64)
      FAIL  forma11 el número de línea es el del archivo — no encontré [form11.pem:2:] en la salida
- verde (restaurado): exit=0 fail=0
...
### AC59 (d) trata el fin de archivo como END
- verde (árbol real): exit=0 fail=0
- mutado:             exit=1 fail=1
      FAIL  forma10 detecta la clave AWS sola en su línea después del BEGIN sin END — no encontré [form10.md:4:] en la salida
- verde (restaurado): exit=0 fail=0

### AC59 (e) clase de base64 laxa (el = en cualquier lugar)
- verde (árbol real): exit=0 fail=0
- mutado:             exit=1 fail=2
      FAIL  forma11 clave=valor dentro de un certificado cerrado - rojo (no es base64)
      FAIL  forma11 el número de línea es el del archivo — no encontré [form11.pem:2:] en la salida
...
### AC59 (f) sin saltear los binarios
- verde (árbol real): exit=0 fail=0
- mutado:             exit=1 fail=1
      FAIL  forma12 un binario no se escanea, igual que antes de AC59
- verde (restaurado): exit=0 fail=0

### AC59 (g) sin tolerar CR al reconocer el bloque
- verde (árbol real): exit=0 fail=0
- mutado:             exit=1 fail=1
      FAIL  forma13 un certificado con CRLF se reconoce - verde
- verde (restaurado): exit=0 fail=0

### AC59 (h) no reabre el bloque ante un segundo BEGIN
- verde (árbol real): exit=0 fail=0
- mutado:             exit=1 fail=2
      FAIL  forma14 un PEM truncado seguido de uno completo no forma un solo bloque - rojo
      FAIL  forma14 detecta la clave AWS entre los dos bloques — no encontré [form14.md:4:] en la salida
- verde (restaurado): exit=0 fail=0

Árbol al terminar: limpio
```

- Las 45 caen, y las 90 corridas verdes, la del árbol real y la restaurada de cada una, salen `exit=0 fail=0`.
...
mutar "AC59 (c) vacía las líneas base64 aunque estén fuera de un certificado" "$SS" "$TSS" \
  '      if (en_cert) { buf[++n] = $0; next }
      print
' '      if (en_cert) { buf[++n] = $0; next }
      if (es_base64($0)) print ""; else print
'
mutar "AC59 (d) trata el fin de archivo como END" "$SS" "$TSS" \
  'END { if (en_cert) for (k = 1; k <= n; k++) print buf[k] }' \
  'END { if (en_cert) for (k = 1; k <= n; k++) { if (k > 1 && es_base64(buf[k])) print ""; else print buf[k] } }'
mutar "AC59 (e) clase de base64 laxa (el = en cualquier lugar)" "$SS" "$TSS" \
  't ~ /^[A-Za-z0-9+\/]+=?=?$/' 't ~ /^[A-Za-z0-9+\/=]+$/'
mutar "AC59 (f) sin saltear los binarios" "$SS" "$TSS" \
  '  grep -Iq . "$REPO_ROOT/$f" 2>/dev/null || continue
' ''
mutar "AC59 (g) sin tolerar CR al reconocer el bloque" "$SS" "$TSS" \
  'function limpia(t) { sub(/\r$/, "", t); sub(/[ \t]+$/, "", t); return t }' \
  'function limpia(t) { sub(/[ \t]+$/, "", t); return t }'
mutar "AC59 (h) no reabre el bloque ante un segundo BEGIN" "$SS" "$TSS" \
...
```
árbol 47a4a69 · bundle sha256 e5bb2084ccf45087
