...
La escalera de arriba se corrió sobre `a95380b`, que ya incluye las correcciones de las rondas 1 y 2 de la review de v25. Las mediciones, las mutaciones y la verificación en vivo se corrieron sobre `c6dbbb7`, el commit que agregó la escalera: el código y los tests son los de `a95380b`. Esta versión del report reemplaza a las de `e62cff6` y `9234fbc`, que quedan en el historial.
...
- Las 34 caen, y las 68 corridas verdes, la del árbol real y la restaurada de cada una, salen `exit=0 fail=0`. Son las 29 de la primera versión y cinco nuevas de las correcciones de las rondas 1 y 2: `AC56 (d)` y `(e)`, y `AC57 (c)`, `(d)` y `(e)`. `AC56 (c)` ahora apaga el chequeo del nombre, que reemplazó al del prefijo. `AC57 (d)` ahora corta antes de redactar.
