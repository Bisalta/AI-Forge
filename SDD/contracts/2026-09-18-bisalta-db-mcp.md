...
- SQL Server sigue igual: cifra sin autenticar al servidor. Queda como **riesgo aceptado** (`D71`): a Dev SQL sólo se llega por la VPN, según confirmó Ian Vargas, y Patrick Ocampo lo aceptó el 25-sep.
...
gradiel12 dejó una review sin veredicto: no bloquea el merge, y pide sus dos primeros puntos antes de habilitar el plugin al equipo. Ian Vargas pidió aplicar lo que se pudiera sin esperar a nadie. Entran cinco ACs nuevos (`AC53` a `AC57`); lo que depende de otra persona queda como deuda con dueño (`D70` a `D76`).

1. **`AC53`**: en SQL Server, la lista blanca rechaza las sentencias que no son lectura y que pueden venir encadenadas sin `;` detrás de un `SELECT`. Es su punto 2, más `WHILE`, que retiene la sesión igual que `WAITFOR`. **Efecto sobre un caso de Patrick**: el caso 8 de `casos-adversariales-lista-blanca.js` usa `OPENQUERY`, y deja de ser límite conocido. Cambia sólo su veredicto esperado, de aceptar a rechazar, con una nota en su descripción; su consulta no se tocó. El límite conocido de v19 se achica: en SQL Server, `OPENQUERY`, `OPENROWSET` y `OPENDATASOURCE` se rechazan por nombre.
2. **`AC54`**: `sqlcmd -t 60`. SQL Server no tiene un límite de sentencia por login; sin esto, una consulta larga retenía la sesión hasta el corte de proceso (125 s). Es el mismo valor que el `statement_timeout` del rol de Postgres.
3. **`AC55`**: TLS obligatorio. Es su punto 1. Postgres, con `PGSSLMODE=require`: medido el 25-sep, la conexión ya iba cifrada con TLS 1.3 por el `prefer` por omisión, pero no hubiera impedido caer a texto plano. SQL Server, con `-N true -C`. Medido el 25-sep: con `-N true` y sin `-C` la conexión falla, porque la instancia presenta `SSL_Self_Signed_Fallback`, el certificado autofirmado que SQL Server genera solo cuando no tiene uno configurado. **Ninguno de los dos verifica todavía el certificado del servidor**: Postgres necesita el bundle de RDS (`D70`), y SQL Server, que Patrick instale o distribuya un certificado verificable (`D71`).
4. **`AC56`**: al arrancar, el servidor borra los directorios temporales del plugin de más de 10 minutos. Es su menor 1: el `finally` no corre si el proceso muere con SIGKILL.
5. **`AC57`**: **un defecto que la review no vio y apareció al medir**. `sqlcmd` escribe sus errores en stdout y deja stderr vacío, y el plugin sólo leía stderr. Todo error de SQL Server llegaba al consumidor con el mensaje vacío, y un corte por `-t` no se hubiera reconocido como `tiempo_agotado`. El stub del test era una copia del de `psql`, que escribe sus errores en stderr, así que ningún test lo podía ver (`RT55`).
6. **Sus menores 2 a 4**: el secreto único por motor pasa a la tabla de riesgos como riesgo aceptado. `COMPRAS_STG` etiquetada `dev` queda para Patrick (`D72`). Y el README dice ahora que la lista de hosts prohibidos es un freno, no la barrera: la barrera es IAM, como ya decía el threat model, punto 1.

Evidencia en `SDD/verification/feat-GEN-108-mcp-bisalta-db-v25.md`.

### Cambios v23 → v24 (la mutación (c) de `AC51` cae, 24-sep-2026)

La regla de los corchetes de SQL Server es de la misma clase que la de los literales `E'…'`: no está para rechazar ataques, sino para **no rechazar consultas legítimas**. Sin ella, una comilla, una comilla doble o un `/*` dentro de un nombre entre corchetes se leen como una construcción que nunca se cierra. Los casos de Patrick no traían ninguna consulta así, y por eso la mutación (c) caía en parte: la mataba sólo el caso 16, que también cae con la (d). Ian pidió no esperar otro caso de Patrick. El planner escribió cinco en `SDD/tests/test_lista_blanca.sh` (`a282e3a`): cuatro nombres entre corchetes con una comilla simple, una comilla doble, un `/*` y un `]]` escapado seguido de una comilla, más un control, un corchete simple. Son consultas legítimas y T-SQL válido, no formas de pasar la lista blanca. Medido el 24-sep sobre `eb3e8bc`, que tiene el mismo código y los mismos tests que `a282e3a`: con la regla apagada caen los cuatro, y el control no. Con la (d), ninguno de los cuatro cae. **La mutación (c) queda cerrada** (`D64`). Se cambia su condición de aprobación en `AC51`, por eso el bump. Evidencia en `SDD/verification/feat-GEN-108-mcp-bisalta-db-v24.md`.

### Cambios v22 → v23 (la mutación (b) de `AC51` cae, 24-sep-2026)

Patrick Ocampo explicó por qué la mutación (b) no caía: la regla de los literales `E'…'` no está para rechazar consultas hostiles —la regla de la construcción sin cerrar ya rechaza las mismas—, sino para **no rechazar consultas legítimas**. Ningún caso de ataque la podía matar. Mandó seis casos de consultas legítimas que tienen que aceptarse; Ian los pegó tal cual en `SDD/tests/fixtures/casos-adversariales-v20.js` (`76f258e`), que pasa de 21 a 27 casos. Medido el 24-sep sobre `76f258e`: con la regla apagada caen cinco (índices 21 a 25), y el sexto (índice 26) es un control que Patrick puso a propósito. **La mutación (b) queda cerrada** (`D63`). Se cambia su condición de aprobación en `AC51`, por eso el bump (regla de v18). Las mutaciones (b), (c) y (d) se corrieron además sobre este árbol; la evidencia está en `SDD/verification/feat-GEN-108-mcp-bisalta-db-v23.md`. La (a), con adaptador, sigue sin correrse sobre el árbol actual (`D65`, en parte). *(Superado: ver el segundo agregado, más abajo.)*
...
**Segundo agregado a v23 (24-sep, sin bump: no cambia ninguna condición de aprobación)**: la mutación (a) se corrió sobre el árbol actual, con el adaptador que ya declara `AC51`, y cae (`D65`). La evidencia está en el addendum de `SDD/verification/feat-GEN-108-mcp-bisalta-db-v23-D67.md`. Las cuatro mutaciones de `AC51` quedan corridas: la (a) sobre el árbol actual; (b), (c) y (d) sobre `76f258e`, y después de ese commit el único cambio en `SDD/tests/` es la descripción del caso 13.
...
Casos de prueba: los 21 de la revisión de Patrick Ocampo (`SDD/tests/fixtures/casos-adversariales-lista-blanca.js`), más los 27 de su segunda tanda (`SDD/tests/fixtures/casos-adversariales-v20.js`: 21 del 23-sep y 6 del 24-sep, que agregó para la mutación (b)). En `0358e73` se corrigió el texto descriptivo del caso 0 de ese archivo, que no cargaba, y en `e0a70bb` el del caso 13, que no nombraba su tipo; ninguna consulta se tocó. Más cinco casos del planner para la mutación (c), en `SDD/tests/test_lista_blanca.sh` (asserts `AC51 (c) acepta en sqlserver …`, v24). **Mutaciones declaradas**: (a) volver a la normalización anterior a v19 pone rojo al menos uno de los casos de Patrick — como la función nueva devuelve un objeto, la versión vieja se sustituye con un adaptador: `{ texto: normalizarVieja(sql), sinCerrar: null }`; (b) quitar la regla de los literales `E'…'` pone rojo al menos uno de los casos 21 a 25 de `casos-adversariales-v20.js` — consultas legítimas que, sin la regla, se rechazan como construcción sin cerrar; el caso 26 es un control y no tiene que caer. (Hasta v22, **abierta**: con los 21 casos del 23-sep, apagar la regla dejaba la suite verde.); (c) quitar la regla de los corchetes pone rojo al menos uno de los asserts `AC51 (c) acepta en sqlserver …` que no son el control. Son consultas legítimas que, sin la regla, se rechazan como construcción sin cerrar; el control (un corchete simple) no tiene que caer. (Hasta v23, **abierta en parte**: caía sólo el caso 16, que también cae con (d).); (d) aceptar una construcción sin cerrar pone rojo su caso. (Reescrito en v20: el texto de v19 enumeraba sólo comillas simples y dobles.)
...
**AC53** (detección, v25) — En dialecto `sqlserver`, una sentencia que contenga en cualquier posición de su texto normalizado `WAITFOR`, `WHILE`, `GRANT`, `REVOKE`, `DENY`, `USE`, `DBCC`, `SET`, `DECLARE`, `BEGIN`, `BACKUP`, `RESTORE`, `KILL`, `SHUTDOWN`, `OPENROWSET`, `OPENQUERY` u `OPENDATASOURCE` como palabra completa se rechaza con motivo `sentencia_no_permitida`. El chequeo va después del de escritura embebida, así que ningún rechazo anterior cambia de motivo. La palabra como parte de un nombre (`begin_date`) o dentro de un literal no cuenta; dentro de un nombre entre corchetes o entre comillas dobles sí, y ese rechazo de más es a sabiendas. Casos de prueba: uno por palabra, con el motivo exacto, más tres de aceptación, en `SDD/tests/test_lista_blanca.sh`. **Mutaciones declaradas**: (a) apagar la regla pone rojo al menos un caso; (b) quitar el borde de palabra de la izquierda pone rojo el caso de aceptación de los nombres; (c) aplicar la regla al SQL sin normalizar pone rojo el caso de aceptación del literal; (d) quitar una palabra cualquiera de la lista pone rojo su caso.
...
**AC57** (detección, v25) — Cuando `sqlcmd` falla con stderr vacío, el texto del error se toma de stdout: se redacta stdout **entero**, y después se toma **desde la primera línea `Msg <n>, Level <n>`** —o, si no hay ninguna, los últimos 500 caracteres— y se corta a 500, como el de `psql`, que también se redacta antes de cortar. Así, si falla después de emitir filas, el mensaje no son datos. Un corte por `-t` se informa como `tiempo_agotado`. En una corrida exitosa, stdout son datos y no se lee como error. **Mutaciones declaradas**: (a) leer el error sólo de stderr pone rojo los asserts del mensaje y del corte; (c) tomar stdout entero, sin buscar el `Msg`, pone rojo los dos asserts del error después de filas; (d) sacar la redacción de stdout entero pone rojo el assert de la credencial partida por el corte; (e) tomar stdout entero cuando no hay `Msg`, y no su final, pone rojo los asserts del tramo final; (b) leer stdout también cuando el proceso no falló pone rojo el de la corrida exitosa. **Parte `manual-only`**: una consulta que falla por permiso en la instancia real devuelve su mensaje.
...
| ~~El cifrado en tránsito no autentica al servidor en ningún motor (v25).~~ En SQL Server el cifrado todavía no autentica al servidor (v26). | Postgres autentica al servidor desde v26 (`AC58`, `D70` pagada). En SQL Server, `AC55` impide el texto plano. La instancia presenta `SSL_Self_Signed_Fallback`, así que no se puede verificar. **Riesgo aceptado** (`D71`, 25-sep): a Dev SQL sólo se llega por la VPN, según confirmó Ian Vargas, y Patrick Ocampo lo aceptó. Lo que no se cubre es una suplantación desde dentro de la red. Si aparece otro camino a la instancia, se reabre. |
