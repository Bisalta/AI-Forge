...
Casos de prueba: los 21 de la revisión de Patrick Ocampo (`SDD/tests/fixtures/casos-adversariales-lista-blanca.js`), más los 21 de su segunda tanda (`SDD/tests/fixtures/casos-adversariales-v20.js`). En `0358e73` se corrigió el texto descriptivo del caso 0 de ese archivo, que no cargaba; su consulta no se tocó. **Mutaciones declaradas**: (a) volver a la normalización anterior a v19 pone rojo al menos uno de los casos de Patrick — como la función nueva devuelve un objeto, la versión vieja se sustituye con un adaptador: `{ texto: normalizarVieja(sql), sinCerrar: null }`; (b) quitar la regla de los literales `E'…'` pone rojo su caso — **abierta**: medido el 24-sep, apagar esa regla deja la suite verde, así que hoy ningún caso la mata. Se le pidió a Patrick el caso que falta; (c) quitar la regla de los corchetes pone rojo sus casos — **abierta en parte**: medido el 24-sep, cae un solo caso, que también cae con (d); (d) aceptar una construcción sin cerrar pone rojo su caso. (Reescrito en v20: el texto de v19 enumeraba sólo comillas simples y dobles.)
