...
**AC56** (detección, v25) — Al arrancar, el servidor borra de su tmpdir los directorios con el nombre que da `mkdtemp` al plugin —el prefijo y seis caracteres— y más de 10 minutos de antigüedad. No toca los recientes, ni los que no son del plugin, ni uno viejo que tenga el prefijo pero no ese nombre, y no sigue symlinks (`lstat`). **Mutaciones declaradas**: (a) quitar la llamada al arrancar; (b) borrar sin mirar la edad; (c) borrar sin mirar el nombre; (d) aceptar cualquier nombre con el prefijo. Cada una pone rojo su assert.

**AC57** (detección, v25) — Cuando `sqlcmd` falla con stderr vacío, el texto del error se toma de stdout, **desde la primera línea `Msg <n>, Level <n>`** —o, si no hay ninguna, de los últimos 500 caracteres—, redactado y cortado a 500 caracteres como el de `psql`. Así, si falla después de emitir filas, el mensaje no son datos. Un corte por `-t` se informa como `tiempo_agotado`. En una corrida exitosa, stdout son datos y no se lee como error. **Mutaciones declaradas**: (a) leer el error sólo de stderr pone rojo los asserts del mensaje y del corte; (c) tomar stdout entero, sin buscar el `Msg`, pone rojo los dos asserts del error después de filas; (d) sacar la redacción de ese camino pone rojo el assert de la credencial; (b) leer stdout también cuando el proceso no falló pone rojo el de la corrida exitosa. **Parte `manual-only`**: una consulta que falla por permiso en la instancia real devuelve su mensaje.
