...
3. **`AC55`**: TLS obligatorio. Es su punto 1. Postgres, con `PGSSLMODE=require`: medido el 25-sep, la conexión ya iba cifrada con TLS 1.3 por el `prefer` por omisión, pero no hubiera impedido caer a texto plano. SQL Server, con `-N true -C`. Medido el 25-sep: con `-N true` y sin `-C` la conexión falla, porque el certificado de la instancia no es de una autoridad en la que confíe el cliente. **Ninguno de los dos verifica todavía el certificado del servidor**: Postgres necesita el bundle de RDS (`D70`), y SQL Server, que Patrick instale o distribuya un certificado verificable (`D71`).
