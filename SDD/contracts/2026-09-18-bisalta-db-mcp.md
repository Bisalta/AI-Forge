...
| **El `secret_id` de las 12 entradas del catálogo no lo acordó nadie.** `bisalta-db/postgres/claude_lectura` y `bisalta-db/sqlserver/bisalta_lectura` salieron del borrador del planner en v1; Patrick Ocampo nunca los vio. Y no siguen la convención que la cuenta ya usa (`dev/sp/db-sist-proveedores`, `onpremise/bd/exactus`, `dev/postgresDB`). | Preguntado a Patrick el 22-sep-2026, junto con dos huecos más: si "una conexión, un secreto" significa un secreto por entrada de catálogo (hoy son 2 para 12) y cómo entra `neo_lectura`, que el runbook manda crear y ningún artefacto del plugin consume. **Bloquea la creación de los secretos**, no el merge. |
