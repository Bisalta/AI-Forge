...
**AC56** (detección, v25) — Al arrancar, el servidor borra de su tmpdir los directorios con el prefijo del plugin y más de 10 minutos de antigüedad; no toca los recientes ni los que no son del plugin. **Mutaciones declaradas**: (a) quitar la llamada al arrancar; (b) borrar sin mirar la edad; (c) borrar sin mirar el prefijo. Cada una pone rojo su assert.

**AC57** (detección, v25) — Cuando `sqlcmd` falla con stderr vacío, el texto del error se toma de stdout, redactado y cortado a 500 caracteres como el de `psql`. Un corte por `-t` se informa como `tiempo_agotado`. En una corrida exitosa, stdout son datos y no se lee como error. **Mutaciones declaradas**: (a) leer el error sólo de stderr pone rojo los asserts del mensaje y del corte; (b) leer stdout también cuando el proceso no falló pone rojo el de la corrida exitosa. **Parte `manual-only`**: una consulta que falla por permiso en la instancia real devuelve su mensaje.
