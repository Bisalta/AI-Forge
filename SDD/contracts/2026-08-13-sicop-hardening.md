...
- **Contract version**: v8 — ratificado el 14-ago-2026 tras el `BLOCKED` de la ronda 1 de R4. El implementador auditó el checklist estadístico del arquetipo `analysis`, como el brief le pidió, y encontró que **dos de los siete ítems eran autosatisfacibles por el error que pretenden prevenir**. Reescritos los ítems 5 y 6 con su razón medida. AC42 se amplía a la regla `test-skipeado`, que tiene el mismo hueco de `.md`. Es el único defecto del ciclo que un review por grep no podía encontrar.
- **Historial de versiones**: v7 — ratificado el 14-ago-2026 tras la re-review de R3 por AC24. El reviewer encontró que este contract incumplía §10.2, la regla insignia que R3 introduce: ningún AC de detección declaraba su `Mutación:`. Agregadas las nueve. Además se corrige el encabezado, que había vuelto a tener dos líneas rotuladas `Contract version` — la misma regresión que el review de R0 ya había marcado una vez.
...
- **Checklist → ACs**:
  ...
  - **Unidad de análisis y unidad de agrupamiento declaradas, con el número de grupos. Cuando las observaciones se agrupan, la inferencia se hace a nivel del grupo, y el reporte nombra la técnica usada.** *(Reescrito en v8. La redacción v1-v7 decía "unidad de análisis declarada, y el test a esa unidad cuando las observaciones se agrupan", y `esa unidad` sólo podía referirse a la declarada: no exigía que fuera aquella en la que las observaciones son independientes. En el propio incidente que motiva el ítem, declarar "unidad = la oferta" lo cumplía **corriendo el test al nivel equivocado**, que es el error que produjo la brecha de 0,215. Tampoco pedía el número de grupos, que es lo que gobierna la inferencia agrupada.)*
  - **Tamaño de muestra (filas y grupos) y tamaño de efecto mínimo detectable declarados, con el número: contra qué efecto, a qué α y a qué potencia. La potencia calculada con el efecto observado no cuenta.** *(Reescrito en v8. La redacción v1-v7 pedía "tamaño de muestra y potencia declarados, con el número", y la potencia no es propiedad del dataset sino función de tamaño de efecto, α y test: sin decir contra qué efecto, el número no significa nada. Peor, calculada después de ver el resultado es potencia observada, una transformación monótona del p-valor — con `p = 0,533` el ítem se cumplía escribiendo "potencia = 0,09", que repite lo que `p` ya dijo. Lo que decide algo es el efecto mínimo detectable: es lo único que separa "no hay efecto" de "no hay datos", que es justo lo que el backtest necesitaba.)*
...
- **AC42** — `plugins/sdd-flow/scripts/sdd-check.sh` no levanta `BLOCKER` de supresor **ni de `test-skipeado`** sobre un archivo `.md`. **Ampliado en v8**: la ronda 1 midió que la regla `test-skipeado` tiene el mismo hueco que la de supresores, con 2 hits sobre documentos normativos del propio plugin (`standards/quality-gates.md` §6.1 y `templates/doc_quality_gates.md`). Es el mismo defecto en la regla de al lado, y arreglar una sola deja el problema vivo. **Ratificación v7**: la regla de supresores no tiene el guard `file !~ /\.md$/` que sí tiene la regla de `--no-verify` en el mismo script, así que el chequeo mecánico del plugin levanta un `BLOCKER` sobre los documentos normativos **del propio plugin** — el `@ts-ignore` que enumera las mitigaciones prohibidas en `commands/sdd-fixes.md` lo dispara. Medido por el reviewer: `sdd-check.sh e5ef90c` sale 2 sobre un texto que existía desde antes del diff. Un checker que se pone rojo sobre la prosa que describe lo que prohíbe va a entrenar al equipo a ignorarlo.
