...
**Suite completa** (obligatoria una vez antes de integrar): `bash SDD/tests/run.sh` — mismo comando que el gate 4: este repo no filtra tests por área, la suite entera corre siempre completa (12 archivos de test tras el ciclo `GEN-101`; el duplicado con el gate 4 está registrado como deuda `D5`, no se corrige). Medido (26-ago-2026, tras contract v7 de GEN-101): ~62s.

**Tiempo esperado de la suite completa**: menor a 90 segundos (revisado en GEN-101 — la cifra de R0 de GEN-94, "~2s", quedó obsoleta al crecer de 3 a 12 archivos de test con triples de mutación reales por AC; medido, no estimado: `test_lint_contract_sections.sh` sola es ~49s de las ~62s totales — son ~30 invocaciones reales del linter por los triples de R4, no un cuelgue). Un timeout muy por encima de 90s acá sigue siendo señal real; **por debajo de esa marca ya no lo es** — la cifra vieja de "<5s" habría hecho sonar una alarma falsa contra este mismo commit.
