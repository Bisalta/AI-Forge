...
**Suite completa** (obligatoria una vez antes de integrar): `bash SDD/tests/run.sh` — mismo comando que el gate 4: este repo no filtra tests por área, la suite entera corre siempre completa (el duplicado con el gate 4 está registrado como deuda `D5`, no se corrige). **Cuántos archivos de test hay se deriva del árbol, no se cita acá**: `ls SDD/tests/test_*.sh | wc -l`. Una cifra congelada en prosa drifta respecto de la realidad que describe — ya pasó: este doc declaraba "12 archivos" cuando había 14, y ese drift obligó a bumpear el contract de GEN-108 de v1 a v2. Medido (18-sep-2026, tras R2 de GEN-108): **27s**, tres corridas consecutivas, las tres en 27s.

**Tiempo esperado de la suite completa**: menor a 90 segundos. La marca se sostiene con la medición de GEN-108 R2 (18-sep-2026): **27s** con los tres archivos de test nuevos del plugin `bisalta-db`, tres corridas consecutivas sin variación. El grueso lo sigue poniendo `test_lint_contract_sections.sh` (~30 invocaciones reales del linter por los triples de R4 de GEN-101), no un cuelgue. Las mediciones anteriores fueron "~2s" (R0 de GEN-94, 3 archivos) y "~62s" (GEN-101); la de hoy es más baja que la de GEN-101 **con más archivos**, así que el número depende de la carga de la máquina y no sólo del contenido de la suite: por eso el umbral se mantiene holgado en vez de ajustarse al último valor medido. Un timeout muy por encima de 90s acá sigue siendo señal real; **por debajo de esa marca ya no lo es**.
...
- `shellcheck` disponible, **verificado con `command -v shellcheck` antes de cada corrida de gates** — no se da por presente. La verificación es lo único que no envejece: el 18-sep-2026 a las 11:26 el binario **no estaba en el PATH** y a las 11:37 **sí**, sin que nadie lo instalara de cero (llevaba un año en el Cellar de Homebrew; lo que faltaba era el symlink, que `brew install shellcheck` recreó). O sea que un doc que afirme "está instalado" describe un estado que se rompe y vuelve. Si `command -v` no lo encuentra, el gate 2 se marca `[SKIPPED] shellcheck no disponible`, **nunca verde**. Última corrida verde medida en esta máquina: v0.11.0, `/opt/homebrew/bin/shellcheck`, 18-sep-2026.
- `node` disponible (`command -v node`; v22.17.0 en esta máquina el 18-sep-2026). Lo exigen los tres tests de `bisalta-db` (`test_lista_blanca.sh`, `test_catalogo.sh`, `test_servidor_mcp.sh`), que invocan los scripts del plugin desde afuera. Sin él, esos tres archivos fallan en su chequeo de setup y el gate 4 sale rojo — **no** se saltean solos. No hay `package.json` ni `node_modules`: el plugin es Node plano, sin dependencias.
