...
- AC10: `grep -ni "no queda cubierta" plugins/bisalta-db/aprovisionamiento/RUNBOOK.md` → coincidencia en línea 261 (declaración explícita del hueco de SQL Server). El comando original (sin `-i`) no matcheaba porque el runbook usa `NO` en mayúsculas — corregido en el mismo paso, antes de declarar el AC en verde.
